...
数据劫持最著名的应用双向绑定，例如 Vue 使用的是 `Object.defineProperty()`（他们在 3.0 版本之后改用 `Proxy`）。此外 immer.js 为了保证数据的 immutable 属性，使用了 `Proxy` 来阻断常规的修改操作，也是数据劫持的一种应用。另外大名鼎鼎的 `(a === 1 && a === 2 && a === 3) === true` 也都可以使用这两个方法来实现。那这两个方法究竟有何区别呢？
...
只谈技术本身而不谈应用场景基本都是耍流氓。一个技术只有拥有了应用场景，才能真正为我们所用。

如开头所说，数据劫持多出现在框架内部，例如 Vue, immer 之类的，好像和我们普通程序员相去甚远。除开这些，我列举几个可能的应用场景，大家在平时的工作中可能还能想到更多。
...
使用 `Proxy` 也可以，但因为 `Proxy` 的语法是返回一个新的对象，因此要做到 `a === 1` 可能比较困难，做到 `obj.a === 1` 还是很轻松的，反正原理是一样的，也不必纠结太多。
...
既然能够操纵 get，自然就可以实现某些属性可以访问，而某些不可以，这就是共有和私有变量的概念。实现起来也很简单：
...
let handler = {
  get (target, key, receiver) {
    ...
    // 如果出错，把错误的 key 传递到错误处理函数 dealError 里面
    if (!target[key]) {
      if (!target['HAS_ERROR']) {
        dealError(key)
      }
...
因为我们故意设置了 `response.data = undefined`，因此会进入 `dealError` 方法，参数 `key` 的值为 `data`。虽然总共看起来比上面的 `if` 检查更长，但实际上 `isOK`, `handler` 和 `new Proxy` 的定义都是可以复用的，可以移动到一个单独的文件，仅暴露几个方法即可。所以实际的代码只有最后的一个 `if` 而已。
