...
解决方案：
1. 两个工具：puppeteer & lighthouse

2. 从 AMP 学到的 web packaging。把网页内容存到离用户更近的位置。把网页内容类似镜像的概念，从远处的原服务器复制到镜像服务器，由游览器识别出来。
...
m.dhgate.com 一个来自中国的B2B电商网站，采用了 AMP。
...
*解释了浏览器的 event loop (异步任务(task)，渲染任务(raf, render steps)，micro tasks)。个人认为是本届最有技术含量的 WEB 讲座*

document.body.appendChild(el)
el.style.display = 'none'

这两句代码的顺序让人觉得会有闪动，因此编码时经常会交换两句的顺序。但实际上无论怎么写，都不会闪动。由此引入了 event loop。

JS 是单线程的（人并没有主线程，也没有阻塞的感觉）。
...
task queue
有异步队列时就执行异步队列，没有就不执行，只在主线程循环。

render steps
实际上是 structure, layout, paint

button.addEventListener('click', () => {
  while(true);
})

这样会导致异步队列被阻塞（永远执行），因此不单单主线程，render steps也同样无法被执行，因此页面无法再选中，也无法再更换内容（但鼠标可以动）
...
每个异步任务的执行效果都是加入一个新的异步任务，因此并不会阻塞任何东西。
...
第二段会比第一段让元素移动快得多。表示 callback 被调用的次数也多很多。这是因为 settimeout 会尽量运行并添加自己到队列。等渲染的时候（16ms) 它已经运行很多次了，所以 render steps 一下会更新很多像素，而不是 1 像素。
常规来说，setTimeout(callback, 4.7) 会和 raf 差不多

setTimeout 并不是用来渲染的，因此和渲染动画相关的，多用raf，不会有掉帧的问题（某一帧没有渲染，第二帧把两次的结果一起渲染）

一段同步代码修改同一个元素的属性，浏览器会直接优化到最后一个。例如
...
本意是从0到1000，然后动画到500。但实际是0直接到500.

解决方法是增加一个获取计算属性在第二行。

microtasks
Promise.then() 内部的就是microtasks

tasks -> animation callbacks -> microtasks

tasks 只执行一个，进入 animation callbacks (settimeout)
animation callbacks 执行全部，但对新增的不执行，因为那是下一个循环
microtasks 直接执行到空队列才继续，包括新增的。因此持续添加会卡住，和settimeout不同。
...
如果在最后加上 button.click() 打印顺序会有所区别
listener 1
listener 2
microtask 1
microtask 2
...
<!-- 需要再查查 -->
原因：
* 用户直接点击的时候，先后触发 2 个 listener。第一个 listener 触发完成 (listener 1) 之后，队列空了，就先打印了 microtask 1。然后再执行下一个 listener
* 脚本执行时，浏览器的内部实现是把 2 个 listener 都同步执行。因此 "listener 1" 之后，执行队列还没空，还要继续执行 "listener 2" 之后才行。所以 listener 2 会早于 microtask 1
